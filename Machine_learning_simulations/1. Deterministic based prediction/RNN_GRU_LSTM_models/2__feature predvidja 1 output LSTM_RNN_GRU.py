...
class LSTMPointProcess(nn.Module):
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (weight.new(self.n_layers, batch_size, self.hidden_dim),
                  weight.new(self.n_layers, batch_size, self.hidden_dim)) #dodao ovde to(device)
        return hidden
...
def train(train_loader, learn_rate, hidden_dim, number_of_layers, EPOCHS=2000, model_type="GRU"):
...
#			PATH_gru = "modelRNN/2features predict 2 outputs GRU.pt"
			...
			simulation_name = '2features predict 1 output (vreme zastoja)' 
