...
def pred(model, testX):
    pred = []
    test_x = testX[0]
    for i in range(len(testX)):
        inp = np.expand_dims(test_x, axis=0)
        out = LSTM_model.predict(inp)
        prediction = out.reshape(2)
        pred.append(prediction)
        a = test_x.reshape(1,-1)
        test_x = np.roll(a, -2)
...
prediction = pred(LSTM_model, testX)
...
# working_len = [i for i in working_len if i <= 3000] #drop outliers
...
np.save('vremena_otkaza_mean_tf.npy', vremena_otkaza)
np.save('vremena_popravke_mean_tf.npy', vremena_popravke)
