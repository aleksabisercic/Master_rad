...
model.add(tf.keras.layers.LSTM(timesteps, activation='relu', input_shape=[n_timestaps, n_features]))
model.add(tf.keras.layers.RepeatVector(n_outputs))
model.add(tf.keras.layers.LSTM(timesteps, activation='relu', return_sequences=True))
model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(25, activation='relu')))
...
model.compile(loss='mse',
              optimizer=optimizer,
              metrics=["mae"])
history = model.fit(train_dataset, epochs=150)# , callbacks=[lr_schedule])
